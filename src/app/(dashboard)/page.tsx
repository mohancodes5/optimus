...
const getDashboardData = unstable_cache(
  async () => {
    ...
    const growth = monthKeys.map((key) => ({
      month: format(new Date(`${key}-01`), "MMM"),
      members: growthByMonthRaw.filter((m) => format(m.startDate, "yyyy-MM") === key)
        .length,
    }));
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <div className="rounded-xl border border-primary/20 bg-gradient-to-br from-accent to-secondary p-5 shadow-sm">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Add members from the Members page, print each member&apos;s QR from their profile, then use
            Check-In for front-desk attendance. Use Alerts for unpaid or expiring reminders.
          </p>
          <ul className="mt-4 space-y-2 text-sm text-muted-foreground">
            ...
            <li className="text-danger">• Red badges = Expired / Overdue / Suspended</li>
          </ul>
        </div>
      </div>
    </div>
